...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/" or self.path.startswith("/?"):
            ...
            conn = get_db()
            rows = conn.execute(
                "SELECT name, status, instagram, facebook, updated_at FROM rsvps WHERE approved = 1 ORDER BY updated_at DESC"
            ).fetchall()
            ...
            conn.close()
            guests = [{"name": r["name"], "status": r["status"], "instagram": r["instagram"] or "", "facebook": r["facebook"] or "", "time": r["updated_at"]} for r in rows]
            for p in plus_ones:
                guests.append({"name": p["name"], "status": "going", "instagram": "", "facebook": "", "time": p["created_at"]})
            ...
            if not name:
                ...
                return
            conn = get_db()
            row = conn.execute("SELECT status, approved FROM rsvps WHERE name = ? COLLATE NOCASE", (name,)).fetchone()
            conn.close()
            if row:
                json_response(self, 200, {"found": True, "status": row["status"], "approved": row["approved"]})
            else:
                json_response(self, 200, {"found": False})
            ...
            if not name:
                ...
                return
            conn = get_db()
            rows = conn.execute("SELECT id, name, approved FROM plus_ones WHERE added_by = ? COLLATE NOCASE ORDER BY created_at DESC", (name,)).fetchall()
            conn.close()
            json_response(self, 200, {"plus_ones": [{"id": r["id"], "name": r["name"], "approved": r["approved"] == 1, "denied": r["approved"] == -1} for r in rows]})
    ...
    def do_POST(self):
        if self.path == "/api/rsvp":
            ...
            if not added_by or not name:
                ...
                return
            conn = get_db()
            # Check if this plus one already exists for this guest
            ...
            if existing:
                ...
                conn.close()
                return
            conn.execute("INSERT INTO plus_ones (added_by, name) VALUES (?, ?)", (added_by, name))
            conn.commit()
            conn.close()
            json_response(self, 201, {"ok": True})
...
<script>
  ...
  async function refreshMyStatus() {
    ...
    if (!name) return;
    // Hide the name input and show name as plain text
    const nameGroup = document.getElementById('name-group');
    nameGroup.innerHTML = '<div class="returning-name">' + escapeHtml(name) + '</div>';
    // Keep a hidden input so submitRsvp can still read the name
...
<script>
  ...
  function renderInviteTable() {
    ...
    tbody.innerHTML = guests.map(g => {
      ...
      const mainRow = '<tr><td><strong>' + esc(g.name) + '</strong></td><td>' + statusHtml + '</td><td><button class="copy-btn" id="' + btnId + '" onclick="copyLink(\'' + esc(inviteUrl).replace(/'/g, "\\'") + '\',\'' + btnId + '\')" title="' + esc(inviteUrl) + '">Copy Link</button></td></tr>';
