...
def rsvp_page():
    ...
    if any(bot in ua for bot in ("facebookexternalhit", "facebookcatalog", "linkedinbot", "twitterbot", "whatsapp")):
...
<script>
  ...
  async function submitRsvp(status) {
    ...
    fetch('/api/rsvp', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, status, open_invite_token: localStorage.getItem('krish_james_party_v2_walkup') || '' })
  ...
  (function() {
    ...
    const isMessenger = /FBAN|FBAV|FB_IAB|FBIOS/.test(ua) && !isInstagram;
    if (!isInstagram && !isMessenger) return;
    const appName = isInstagram ? 'Instagram' : 'Messenger';
    const overlay = document.createElement('div');
    overlay.className = 'iab-overlay';
    const currentUrl = window.location.href;
    overlay.innerHTML = '<div class="iab-card"><div class="iab-icon">🌐</div><div class="iab-title">Open in Browser</div><div class="iab-msg">For the best experience, open this link in your default browser.</div><button class="iab-open-btn" id="iab-copy-btn" onclick="navigator.clipboard.writeText(\'' + currentUrl + '\').then(()=>{document.getElementById(\'iab-copy-btn\').textContent=\'✓ Link Copied!\';document.querySelector(\'.iab-msg\').textContent=\'Now open your browser and paste the link in the address bar.\';}).catch(()=>{document.querySelector(\'.iab-msg\').textContent=\'Copy this link and paste it in your browser: ' + currentUrl + '\'})">Copy Link</button><span class="iab-dismiss" onclick="this.closest(\'.iab-overlay\').classList.add(\'hide\');setTimeout(()=>this.closest(\'.iab-overlay\').remove(),250)">Continue in ' + appName + '</span></div>';
...
<script>
  ...
  function renderWalkupLinks(tokens) {
    ...
    list.innerHTML = '<div style="display:flex;flex-direction:column;gap:8px">'
      + tokens.map(t => {
        ...
        return '<div style="display:flex;align-items:center;gap:10px;background:#f0eee9;padding:6px 6px 6px 14px;border-radius:100px" id="wu-row-' + t.id + '">'
          + '<input type="text" value="' + nameVal + '" placeholder="Type their name..." onkeydown="if(event.key===\'Enter\'){saveWalkupLabel(' + t.id + ',this);}" style="width:200px;flex-shrink:0;border:none;background:#fff;border-radius:100px;padding:8px 16px;font-size:14px;font-weight:500;color:' + nameColor + ';outline:none;box-shadow:0 1px 4px rgba(0,0,0,0.07)" oninput="this.style.color=\'#1a1a1a\'">'
          + '<button onclick="saveWalkupLabel(' + t.id + ', this.parentElement.querySelector(\'input\'))" title="Save name" style="width:34px;height:34px;padding:0;background:#27ae60;color:#fff;border:none;border-radius:50%;font-size:16px;cursor:pointer;flex-shrink:0;display:flex;align-items:center;justify-content:center">✓</button>'
  ...
  async function saveWalkupLabel(id, inputEl) {
    const name = inputEl.value.trim();
    const tickBtn = inputEl.nextElementSibling;
    tickBtn.textContent = '…';
    try {
      ...
      inputEl.style.color = name ? '#1a7a42' : '#999';
      tickBtn.textContent = '✓';
      setTimeout(() => { tickBtn.textContent = '✓'; }, 1000);
    } catch(e) { tickBtn.textContent = '✓'; }
  }
  ...
  function copyLink(url, btnId) {
    const text = url + "\n\nHelloo! We're having a party soon! click the link for all the info, +1's, and to RSVP! hope you can make it! ill also add you to the facebook event as a backup :)";
